...
def render_office(df):
    st.markdown("<h2 class='sub-header'>OFFICES</h2>", unsafe_allow_html=True)
    ...
    specific_positions = [2, 3, 4, 6, 8, 9, 10, 11, 12, 18, 19, 20]
    ...
    whatsapp_columns = [df.columns[2], df.columns[3], df.columns[4], df.columns[6], df.columns[8], df.columns[10], df.columns[11], df.columns[12]]
    ...
    if st.session_state.filters_applied:
        ...
        for index, row in st.session_state.filtered_df.iterrows():
            ...
            whatsapp_message = f"Check out this Office:\n\n{property_details}"
...
def share_selected_properties(selected_properties, custom_column_names, whatsapp_columns):
    ...
    whatsapp_message = "Check out these Office properties:\n\n"
