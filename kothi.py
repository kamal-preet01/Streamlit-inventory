...
def render_kothi(df):
    st.markdown("<h2 class='sub-header'>KOTHIS</h2>", unsafe_allow_html=True)

    # Create filters for specific column
    specific_positions = [2, 5, 6, 7, 8, 9, 10, 11, 12, 13, 19, 23]
    ...
    # Custom column names with emojis
    custom_column_names = {
        df.columns[2]: "Location",
        df.columns[5]: "Size",
        df.columns[6]: "SQFT Built-Up Area",
        df.columns[7]: "Floors",
        df.columns[8]: "Kothi type",
        df.columns[9]: "Accommodation",
        df.columns[10]: "Description",
        df.columns[11]: "Purpose",
        df.columns[12]: "Facing",
        df.columns[13]: "Condition",
        df.columns[14]: "Lift",
        df.columns[15]: "STILT (Parking)",
        df.columns[16]: "POOL️",
        df.columns[17]: "PB (Power Backup)",
        df.columns[18]: "HOME THEATER ",
        df.columns[19]: "Rent",
    }
    ...
    if st.button("Apply Filters"):
        ...
        for _, row in filtered_df.iterrows():
            ...
            whatsapp_columns = [df.columns[2], df.columns[5], df.columns[6], df.columns[7], df.columns[8], df.columns[9], df.columns[10], df.columns[11], df.columns[12], df.columns[13], df.columns[14], df.columns[15], df.columns[16], df.columns[17], df.columns[18], df.columns[19]]  # Custom columns for WhatsApp message
            ...
            whatsapp_message = f"Check out this Kothi:\n\n{property_details}"
