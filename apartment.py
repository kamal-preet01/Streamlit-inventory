...
def render_apartment(df):
    st.markdown("<h2 class='sub-header'>APPARTMENTS</h2>", unsafe_allow_html=True)
    ...
    if st.button("Apply Filters"):
        ...
        for _, row in filtered_df.iterrows():
            ...
            whatsapp_columns = [df.columns[2], df.columns[3], df.columns[4], df.columns[5], df.columns[6], df.columns[7], df.columns[8], df.columns[12], df.columns[13], df.columns[14]]  # Custom columns for WhatsApp message
